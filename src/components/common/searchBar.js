...
function SearchBar(props) {
  let {
    ...
    isRightIcon = false,
    findText,
    onRightIconClick = () => {},isSearchActive=false,
  } = props;
  const [searchText, setsearchText] = useState('');
  let userProp={
    inputText:'',
    data:null,
    keyToSearch:'',isActive:isSearchActive,
  }
  return (
        ...
        {isTitle == true ? (
          <View style={[styles.section, {flex: 3}]}>
            <View style={styles.capTitle}>
              <TextInput
                style={{fontFamily: FONTS.LIGHT, fontSize: RFValue(17),borderBottomWidth:1,}}
                editable={true}
                value={searchText} textAlignVertical={'center'}
                maxLength={50} textAlign={'center'}
                placeholder={''}
                placeholderTextColor={GREY}
                onBlur={() => {}}
                underlineColorAndroid={'transparent'}
                onChangeText={title => (setsearchText(title),findText({...userProp,'inputText':title,}))}
                onSubmitEditing={() => findText({...userProp,'inputText':searchText,})}
              />
            </View>
          </View>
        ) : null}
